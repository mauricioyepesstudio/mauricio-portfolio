...
5. Flag anything that touches curation judgment (which assets/campaigns to feature, whether an old project should go live) back to the owner — that's an editorial decision, not one this agent or its specialists make. Same for anything that isn't actually about this codebase (legal, finance, external platform accounts) — that's out of scope for every specialist here, not something to force into one.
